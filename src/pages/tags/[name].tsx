...
const getPostsByTag = (
  ...
  fetch(
    `${process.env.NEXT_PUBLIC_HOST}/api/v1/posts?page=${page}&page-size=${pageSize}&is-draft=${isDraft}&tag=${tagName}&sort=-publishedAt`,
  ).then((res) => res.json());
  ...
  return (
    <main>
      <div className="flex flex-col items-center justify-center py-2">
        <div className="flex flex-none md:flex-row flex-col align-top mt-10 max-w-screen-xl lg:w-9/12 ">
          <ArticleList
            title={`${tagName}の記事`}
            posts={data.posts}
            count={data.count}
